...
def part_1():
    with open('day_06/input.txt') as f:
        times = map(int, re.findall(r'\d+', f.readline()))
        distances = map(int, re.findall(r'\d+', f.readline()))
    lines = list(zip(times, distances))
    ...
    for line in lines:
        ...
        for t in range(1, time):
            total += 1 if (time - t) * t > distance else 0
        
        ans = total if ans == 0 else ans * total
