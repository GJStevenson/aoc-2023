...
def part_1(lines):
    ...
    for line in lines:
        winning, have = line
        winning_num_len = len(list(set(have).intersection(set(winning))))
        total += pow(2, winning_num_len - 1) if winning_num_len > 0 else 0
    return total
...
def part_2(lines):
    ...
    for i, line in enumerate(lines):
        winning, have = line
        winning_num_len = len(list(set(have).intersection(set(winning))))
        for j in range(i + 1, i + min(winning_num_len, len(lines)) + 1):
            copies[j] += 1 + copies[i] 

        total += winning_num_len + (copies[i] * winning_num_len)
...
def read_input():
    with open('day_04/input.txt') as f:
        ...
        for line in f.readlines():
            ...
            res.append((list(map(int, re.findall(r'\d+', winning)))[1:], list(map(int, re.findall(r'\d+', have)))))
        return res 
